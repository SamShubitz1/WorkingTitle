...
func move_player(delta) -> void:
    if not positionDirty:
        return
    var thing = move_and_collide(velocity * delta)
    ...
    if (direction == "left" and position.x-1 <= ((destination_grid_x * grid_pixel_size) + player_offset_x)) or (direction == "right" and position.x+1 >= ((destination_grid_x * grid_pixel_size) + player_offset_x)):
        position.x = int((destination_grid_x * grid_pixel_size) + player_offset_x)
        position.y = int((destination_grid_y * grid_pixel_size) + player_offset_y)
        positionDirty = false
        ...
        print("forced x pos to: " + str(int((destination_grid_x * grid_pixel_size) + player_offset_x)))
        print("player grid spot: " + str(player_grid_x) + ", " + str(player_grid_y))
        print("player raw coords: " + str(position.x) + ", " + str(position.y))
        ...
        player_grid_x = destination_grid_x
        player_grid_y = destination_grid_y
        print("forced y pos to: " + str(int((destination_grid_y * grid_pixel_size) + player_offset_y)))
        print("player grid spot: " + str(player_grid_x) + ", " + str(player_grid_y))
        print("player raw coords: " + str(position.x) + ", " + str(position.y))
...
func update_player_grid_coords() -> void:
    player_grid_x = int((position.x - player_offset_x) / grid_pixel_size)
    player_grid_y = int((position.y - player_offset_y) / grid_pixel_size)
