...
func _ready() -> void:
	...
	selected_button = current_menu[selected_button_index]
...
func _input(_e) -> void:
	if Input.is_action_just_pressed("navigate_backward"):
		if selected_button_index == 0:
			selected_button_index += (current_menu.size() - 1)
		...
		selected_button_index = (selected_button_index + 1) % current_menu.size()
		update_selected_button()
...
func update_selected_button() -> void:
	...
	if current_menu == abilities_menu:
		if selected_button_index <= character_abilities.size() - 1:
			ability_description_label.text = character_abilities[selected_button_index].description
		else:
