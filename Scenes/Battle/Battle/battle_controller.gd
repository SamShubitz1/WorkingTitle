...
func build_characters() -> void:
	...
	var mandrake_abilities = ["Clobber", "Bite"]
	mandrake.init("Mandrake", GameData.Alliance.ENEMY, mandrake.get_node("CharSprite"), mandrake.get_node("CharHealth"), 300, mandrake_abilities, Vector2i(6, 1)) # init props will be accessed from somewhere
